...
def process_vctk(csv_file):
    ...
    for i, f in enumerate(file_ids):
        ...
        # get label index
        label = w2v.scentence2index(open(_data_path + 'VCTK-Corpus/txt/%s/' % f[:4] + f + '.txt').read())
            ...
            for txt in txt_list:
                with open(txt, 'rt') as f:
                    ...
                    for record in records:
                        ...
                        # label index
                        labels.append(w2v.scentence2index(' '.join(field[1:])))  # last column is text label
    ...
    for stm in stm_list:
        with open(stm, 'rt') as f:
            ...
            for record in records:
                ...
                # label index
                labels.append(w2v.scentence2index(' '.join(field[6:])))
